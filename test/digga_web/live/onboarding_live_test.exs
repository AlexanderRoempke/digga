defmodule DiggaWeb.OnboardingLiveTest do
  use DiggaWeb.ConnCase

  import Phoenix.LiveViewTest

  def setup_onbarding_account(%{conn: conn, user: user}) do
    account = Digga.Users.with_personal_account(user).personal_account
    Digga.Accounts.update_account(account, %{onboarding_required: true})
  ...
  describe "onboarding" do
    ...
    test "when onboarding is required, it should redirect", %{conn: conn} do
      assert {:error, {:redirect, %{to: path}}} = live(conn, ~p"/requires_onboarding")
      assert path =~ "/onboarding/step-1"
    end

    test "when completing a step, it gets redirected", %{conn: conn} do
      assert {:ok, view, _html} = live(conn, ~p"/onboarding/step-1")
