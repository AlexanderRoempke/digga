defmodule Digga.Users do
  ...
  alias Digga.Users.{User, UserToken, UserNotifier}
  ...
  defp create_personal_account({:ok, user} = result) do
    {:ok, account} =
      Digga.Accounts.create_account(
        user, %{personal: true, name: "Personal Account"}
      )

    Digga.Accounts.create_membership(account, user, %{role: :owner})
  ...
  def generate_authenticator_url(user) do
    ...
    issuer = Application.get_env(:digga, :app_name, "Digga")
