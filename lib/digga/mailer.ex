defmodule Digga.Mailer do
  defmacro __using__(_opts \\ []) do
    quote do
      use Swoosh.Mailer, otp_app: :digga
      import Swoosh.Email
      import Digga.Mailer
    end
  end
  ...
  def render_body(email, template, args) when is_atom(template) and is_map(args) do
    heex = apply(DiggaWeb.EmailHTML, template, [args])
    html_with_layout = render_component(DiggaWeb.EmailHTML.layout(%{inner_content: heex}))
  ...
  def premail(email) do
    ...
    text_with_layout = render_component(DiggaWeb.EmailHTML.layout_text(%{inner_content: text}))
  ...
  defp from_email, do: Application.get_env(:digga, :from_email) || "noreply@example.com"
end
