defmodule DiggaWeb.Admin.ResetPasswordController do
  use DiggaWeb, :controller

  alias Digga.{Admins, Admins.AdminNotifier, Mailer}

  plug DiggaWeb.Plugs.RedirectAdmin
  ...
  def create(conn, %{"admin" => %{"email" => email}}) do
    if admin = Admins.get_admin_by_email(email) do
      token = Phoenix.Token.sign(DiggaWeb.Endpoint, "admin_auth", admin.id)
  ...
  def show(conn, %{"token" => token}) do
    case Phoenix.Token.verify(DiggaWeb.Endpoint, "admin_auth", token, max_age: 600) do
      {:ok, id} ->
