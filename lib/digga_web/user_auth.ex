defmodule DiggaWeb.UserAuth do
  @moduledoc false
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_digga_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      DiggaWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule DiggaWeb.PageLive do
        use DiggaWeb, :live_view

        on_mount {DiggaWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{DiggaWeb.UserAuth, :ensure_authenticated}] do
  ...
  defp ensure_2fa_setup(conn) do
    if Application.get_env(:digga, :require_2fa_setup) && conn.assigns[:current_user] do
