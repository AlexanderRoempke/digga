defmodule DiggaWeb.UserTwoFactorLive do
  ...
  def render(assigns) do
    ...
    <div class="space-y-2">
      ...
      <.simple_form :let={f} for={%{}} as={:user} id="verify-2fa-auth" phx-submit="verify">
        <.input field={{f, :verification}} type="text" value="" name="twofactorauth" required />
        <:actions>
  ...
  def handle_event("verify", %{"twofactorauth" => challenge}, socket) do
    ...
    if Users.verify_timebased_challenge(user, challenge) do
      ...
      token = Phoenix.Token.sign(DiggaWeb.Endpoint, "2fa_confirmed", "true")
